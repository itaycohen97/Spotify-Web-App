...
import flask_login
from flask import Flask, redirect, make_response, render_template, request, url_for, send_from_directory
...
def home(error=""):
    if current_user.is_authenticated:
        ...
        return render_template("home.html", user=current_user.user_personal_data, animations=True)
...
def logged_in():
    if 'code' in request.args:
        ...
        print("got user data, putting in cache")
        if user:
            flask_login.login_user(user, remember=False, duration=datetime.timedelta(seconds=user.time_to_expire()),
                                   force=False, fresh=True)
            print("user logged in and in cache")
            return redirect('/')
    else:
...
def recommend():
    if request.method == 'POST':
        ...
        if songlist:
            ...
            result = make_response(render_template("view_songs.html", user=current_user.user_personal_data, headline=headline,
                                                   songlist=songlist, playlist_url=True))
            result.set_cookie('tracks_uri', tracks_uri)
            return result
        else:
            headline = "Sorry, Something Went Wrong..."
            return render_template("home.html", user=current_user.user_personal_data, headline=headline)

    else:
        genre = get_genres(header=current_user.get_headers())
        return render_template("rec_form.html", user=current_user.user_personal_data, genre=genre)


@app.route('/makeplaylist')
@login_required
def makeplaylist():
    data = make_playlist(header=current_user.get_headers(), user_data=current_user,
                         uris=request.cookies.get('tracks_uri'))
    headline = "Here Are Some Songs We Think You'll Like!"
...
def topartists():
    ...
    return render_template("view_artists.html", user=current_user.user_personal_data, headline=headline,
                           artists=top_artists)
...
def toptracks():
    ...
    headline = "Top Tracks"
    return render_template("view_songs.html", user=current_user.user_personal_data, headline=headline,
                           playlists=top_tracks)

@app.route('/doubletoptracks')
@login_required
def doubletoptracks():
    top_tracks = get_double_top_tracks(header=current_user.get_headers())
