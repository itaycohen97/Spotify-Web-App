...
def get_token(code):
        ...
        headers = {
            ...
            "Authorization": "Basic " + get_client_credentials()
        }
        body = {
        ...
        data['expires'] = expires.strftime("%m/%d/%Y, %H:%M:%S")
        data['header'] = {
...
def get_user_data(header):
        request = requests.get("https://api.spotify.com/v1/me", headers=header)
        if request.status_code not in range(200, 299):
            print(str(request.status_code) + "is the status code for get user data")
            return False
        data = request.json()
...
def make_playlist(header, user_data, uris):
        request = requests.post(f"https://api.spotify.com/v1/users/{user_data['user_id']}/playlists",
                                headers=header,
